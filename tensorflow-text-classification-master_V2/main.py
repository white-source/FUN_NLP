...
def train():
    ...
    print("定义优化器。。。\n")
    # 定义优化函数，传入学习速率参数
    optimizer = tf.compat.v1.train.AdamOptimizer(config.trainConfig.learningRate)
    # 计算梯度,得到梯度和变量
    ...
    for epoch in range(config.trainConfig.epoches):
        ...
        for x_batch, y_batch in batch_train:
            ...
            if currentStep % config.trainConfig.print_per_step == 0:
                print(
                    "train: step: {}, loss: {:.6f}, acc: {:.4f}, recall: {:.4f}, precision: {:.4f}, f_beta: {:.4f}".format(
            ...
            if currentStep % config.trainConfig.evaluateEvery == 0:
                ...
                print(
                    "{}, step: {}, loss: {:.6f}, acc: {:.4f},precision: {:.4f}, recall: {:.4f}, f_beta: {:.4f} {}".format(
...
def test(test_data, test_label):
    ...
    saver = tf.compat.v1.train.import_meta_graph(os.path.join(test_save_path, "best_validation.meta"))
...
def predict(data, label, p_data):
    ...
    # save_path = os.path.join(PATH,'checkpoint/bilstm')
    saver = tf.compat.v1.train.import_meta_graph(os.path.join(predict_save_path, "best_validation.meta"))
    with tf.compat.v1.Session() as sess:
        # sess.run(tf.global_variables_initializer())
        saver.restore(sess, os.path.join(predict_save_path, "best_validation"))  # 读取保存的模型
