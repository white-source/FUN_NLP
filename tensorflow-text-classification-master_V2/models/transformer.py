...
class Transformer(object):
    ...
    def __init__(self, config, wordEmbedding):
        ...
        self.inputY = tf.compat.v1.placeholder(tf.int32, [None, config.numClasses], name="inputY")
        ...
        with tf.compat.v1.name_scope("wordEmbedding"):
          self.W = tf.Variable(tf.cast(wordEmbedding, dtype=tf.float32, name="word2vec"), name="W")
          self.wordEmbedded = tf.nn.embedding_lookup(params=self.W, ids=self.inputX)
        
        with tf.compat.v1.name_scope("positionEmbedding"):
          if tf.shape(input=self.wordEmbedded)[0] == config.batchSize:
            self.positionEmbedded = self._positionEmbedding()
          else:
            self.positionEmbedded = self._positionEmbedding(lastBatch=tf.shape(input=self.wordEmbedded)[0])
        ...
        with tf.compat.v1.name_scope("transformer"):
          for i in range(config.modelConfig.numBlocks):
            with tf.compat.v1.name_scope("transformer-{}".format(i + 1)):
        ...
        with tf.compat.v1.name_scope("dropout"):
            outputs = tf.nn.dropout(outputs, rate=1 - (self.dropoutKeepProb))
        ...
        with tf.compat.v1.name_scope("output"):
            ...
            if config.numClasses == 1:
                ...
                self.predictions = tf.argmax(input=self.logits, axis=-1, name="predictions")
        ...
        with tf.compat.v1.name_scope("loss"):
            ...
            if config.numClasses == 1:
                ...
                losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=tf.stop_gradient(self.inputY))
                
            self.loss = tf.reduce_mean(input_tensor=losses) + config.modelConfig.l2RegLambda * l2Loss
    ...
    def _multiheadAttention(self, rawKeys, queries, keys, numUnits=None, causality=False, scope="multiheadAttention"):
        ...
        Q = tf.compat.v1.layers.dense(queries, numUnits, activation=tf.nn.relu)
        K = tf.compat.v1.layers.dense(keys, numUnits, activation=tf.nn.relu)
        V = tf.compat.v1.layers.dense(keys, numUnits, activation=tf.nn.relu)
        ...
        similary = tf.matmul(Q_, tf.transpose(a=K_, perm=[0, 2, 1]))
        ...
        # 将每一时序上的向量中的值相加取平均值
        keyMasks = tf.sign(tf.abs(tf.reduce_sum(input_tensor=rawKeys, axis=-1)))  # 维度[batch_size, time_step]
        print(keyMasks.shape)

        keyMasks = tf.tile(keyMasks, [numHeads, 1]) 

        # 增加一个维度，并进行扩张，得到维度[batch_size * numHeads, queries_len, keys_len]
        keyMasks = tf.tile(tf.expand_dims(keyMasks, 1), [1, tf.shape(input=queries)[1], 1])
        ...
        maskedSimilary = tf.compat.v1.where(tf.equal(keyMasks, 0), paddings, scaledSimilary) # 维度[batch_size * numHeads, queries_len, key_len]
        ...
        if causality:
            ...
            masks = tf.tile(tf.expand_dims(tril, 0), [tf.shape(input=maskedSimilary)[0], 1, 1])  # [batch_size * numHeads, queries_len, keys_len]

            paddings = tf.ones_like(masks) * (-2 ** (32 + 1))
            maskedSimilary = tf.compat.v1.where(tf.equal(masks, 0), paddings, maskedSimilary)  # [batch_size * numHeads, queries_len, keys_len]
        ...
        outputs = tf.nn.dropout(outputs, rate=1 - (keepProb))
    ...
    def _positionEmbedding(self, lastBatch=None, scope="positionEmbedding"):
        ...
        positionEmbedded = tf.nn.embedding_lookup(params=positionEmbedding_, ids=positionIndex)
