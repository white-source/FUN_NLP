    ...
    def build_model(self):
        ...
        for i, filter_size in enumerate(self.config.textCNNConfig.filter_sizes):
            with tf.compat.v1.name_scope("conv-maxpool-%s" % filter_size):
                ...
                conv_w = tf.Variable(tf.random.truncated_normal(filter_shape, stddev=0.1), name="conv_w")
        ...
        with tf.compat.v1.name_scope("dropout"):
            h_drop = tf.nn.dropout(h_pool_flat, 1 - (self.dropoutKeepProb))
        ...
        with tf.compat.v1.name_scope("output"):
            ...
            self.logits = tf.compat.v1.nn.xw_plus_b(h_drop, output_w, output_b, name="logits")
