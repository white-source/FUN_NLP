...
class RcnnModel:
    ...
    def get_predictions(self):
      ...
      if self.config.numClasses == 1:
          ...
          predictions = tf.argmax(input=self.logits, axis=-1, name="predictions")
      return predictions
    ...
    def build_model(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            ...
            if self.wordEmbedding is not None:
                ...
                embedding_w = tf.compat.v1.get_variable("embedding_w", shape=[self.config.vocab_size+2, self.config.rcnnConfig.embedding_size],
                                              initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
            # 利用词嵌入矩阵将输入的数据中的词转换成词向量，维度[batch_size, sequence_length, embedding_size]
            embedded_words = tf.nn.embedding_lookup(params=embedding_w, ids=self.inputX)
            embedded_words_ = embedded_words

        # 定义两层双向LSTM的模型结构
        with tf.compat.v1.name_scope("Bi-LSTM"):
            for idx, hidden_size in enumerate(self.config.rcnnConfig.hidden_dim):
                with tf.compat.v1.name_scope("Bi-LSTM" + str(idx)):
                    # 定义前向LSTM结构
                    lstm_fw_cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(
                        tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=hidden_size, state_is_tuple=True),
                        output_keep_prob=self.dropoutKeepProb)
                    # 定义反向LSTM结构
                    lstm_bw_cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(
                        tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=hidden_size, state_is_tuple=True),
                        output_keep_prob=self.dropoutKeepProb)
                    ...
                    outputs, current_state = tf.compat.v1.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell,
        ...
        with tf.compat.v1.name_scope("context"):
            shape = [tf.shape(input=fw_output)[0], 1, tf.shape(input=fw_output)[2]]
        ...
        with tf.compat.v1.name_scope("wordRepresentation"):
        ...
        with tf.compat.v1.name_scope("text_representation"):
            output_size = self.config.rcnnConfig.output_size
            text_w = tf.Variable(tf.random.uniform([word_size, output_size], -1.0, 1.0), name="text_w")
        ...
        output = tf.reduce_max(input_tensor=text_representation, axis=1)

        # 全连接层的输出
        with tf.compat.v1.name_scope("output"):
            output_w = tf.compat.v1.get_variable(
                "outputW",
            ...
            self.logits = tf.compat.v1.nn.xw_plus_b(output, output_w, output_b, name="logits")
