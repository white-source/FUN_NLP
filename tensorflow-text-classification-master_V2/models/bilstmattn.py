...
class BiLstmAttnModel:
    ...
    def build_model(self):
        ...
        with tf.compat.v1.name_scope("embedding"):
            ...
            if self.wordEmbedding is not None:
                ...
                embedding_w = tf.compat.v1.get_variable("embedding_w", shape=[self.config.vocab_size+2, self.config.biLstmAttnConfig.embedding_size],
    ...
    def _attention(self, H):
        ...
        # 初始化一个权重向量，是可训练的参数
        W = tf.Variable(tf.random.normal([hidden_size], stddev=0.1))
        ...
        r = tf.matmul(tf.transpose(a=H, perm=[0, 2, 1]), tf.reshape(self.alpha, [-1, self.config.sequenceLength, 1]))
        ...
        output = tf.nn.dropout(sentenceRepren, 1 - (self.dropoutKeepProb))
