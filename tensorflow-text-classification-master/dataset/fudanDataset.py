...
class FudanDataset(object):
    ...
    def _contentlabel_to_txt(self, txt_path, content_path, label_path):
        ...
        for txt in files.readlines():  # 读取每一行的txt
            ...
            # print(label)
            # 以下用于获取所有文本
            fp1 = open(txt, "r", encoding="gb18030",
                       errors='ignore')  # 以gb18030的格式打开文件，errors='ignore'用于忽略掉超过该字符编码范围的字符
    ...
    def _getTrainValData(self, dataPath, labelPath):
        ...
        self.trainData, self.valData, self.trainLabels, self.valLabels = train_test_split(trainData, trainLabel,
                                                                                          test_size=self.rate,
                                                                                          random_state=1)
